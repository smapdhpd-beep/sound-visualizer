...
function simulate(count, bass, high, mid, centroid, activity, frames = 800) {
  ...
  for(let f=0;f<frames;f++){
    for(const p of particles){
      const field=waveField(p.x,p.y,sources,freqs,5.0,waveSpeed); // 固定 time=5.0
      const val=field.val;
      if(strength>0.5){
...
draw(simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800), 'sf-bass');
draw(simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800), 'sf-high');
draw(simulate(5, 0.6, 0.7, 0.8, 0.6, 1.0, 800), 'sf-mix');
