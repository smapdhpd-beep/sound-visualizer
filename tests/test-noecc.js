...
function getSources(count, breathe, sourceRotation, time) {
  const sources = [];
  for (let i = 0; i < count; i++) {
    const baseAngle = (Math.PI * 2 * i) / count - Math.PI / 2;
    const localRot = sourceRotation * (1 + Math.sin(i * 1.7) * 0.35);
    const angle = baseAngle + localRot; // 无 eccentricity 扭曲
...
function simulate(count, bass, high, mid, centroid, activity, frames = 800) {
  ...
  for(let f=0;f<frames;f++){
    ...
    const sources=getSources(count,breathe,sourceRotation,waveTime);
...
draw(simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800), 'noecc-bass');
draw(simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800), 'noecc-high');
draw(simulate(5, 0.6, 0.7, 0.8, 0.6, 1.0, 800), 'noecc-mix');
