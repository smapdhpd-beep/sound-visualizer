...
function simulate(count, bass, high, mid, centroid, activity, frames = 800) {
  ...
  const strength=400*activity; // 大幅提高推力
  const brownian=50*(1-activity);
  const damp=Math.pow(0.94,1); // 提高阻尼，减少振荡
  const waveSpeed=130;
  const dt=1/60;
...
draw(simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800), 'slow-bass');
draw(simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800), 'slow-high');
draw(simulate(5, 0.6, 0.7, 0.8, 0.6, 1.0, 800), 'slow-mix');
