...
function simulate(count, bass, high, mid, centroid, activity, frames, name, opts={}) {
  ...
  const glowThresh = opts.glowThresh || 0.35;
  const glowScale = opts.glowScale || 2.2;
  const glowAlpha = opts.glowAlpha || 0.22;
  ...
  for(let f=0;f<frames;f++){
    ...
    ctx.fillStyle=`rgba(10,14,16,${bgAlpha})`;
    ctx.fillRect(0,0,W,H);
    ...
    for(const p of particles){
      ...
      if(acc>glowThresh){
        ...
        const gh=glow*0.5;
        ctx.fillStyle=`rgba(170,180,185,${alpha*glowAlpha})`;
...
// 基准
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'glow-base-bass', {});
// 更低光晕阈值 + 更大光晕
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'glow-low-bass', {glowThresh:0.15, glowScale:3.5, glowAlpha:0.18});
// lighter blend
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'glow-lit-bass', {blend:'lighter'});
// 组合：低阈值 + 大光晕 + lighter
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'glow-all-bass', {glowThresh:0.15, glowScale:3.5, glowAlpha:0.18, blend:'lighter'});
// high freq
simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800, 'glow-all-high', {glowThresh:0.15, glowScale:3.5, glowAlpha:0.18, blend:'lighter'});
