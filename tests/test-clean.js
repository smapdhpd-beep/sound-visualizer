...
function getSources(count, breathe, sourceRotation) {
  const sources = [];
  for (let i = 0; i < count; i++) {
    const baseAngle = (Math.PI * 2 * i) / count - Math.PI / 2;
    const angle = baseAngle + sourceRotation;
...
function simulate(count, bass, high, mid, centroid, activity, frames = 800) {
  ...
  for(let f=0;f<frames;f++){
    ...
    sourceRotation+=dt*0.03; // 极慢自转
    const sources=getSources(count,breathe,sourceRotation);
...
draw(simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800), 'clean-bass');
draw(simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800), 'clean-high');
draw(simulate(5, 0.6, 0.7, 0.8, 0.6, 1.0, 800), 'clean-mix');
