...
function getSources(count, breathe, waveTime, wobble, eccentricity) {
  const sources = [];
  for (let i = 0; i < count; i++) {
    const baseAngle = (Math.PI * 2 * i) / count - Math.PI / 2;
    const angle = baseAngle + Math.sin(waveTime * 0.08 + i * 0.7) * wobble + Math.sin(waveTime * 0.15 + i * 1.3) * eccentricity;
...
function simulate(count, bass, high, mid, centroid, activity, frames = 800) {
  const particles=[];
  for(let i=0;i<2000;i++) particles.push({x:Math.random()*W,y:Math.random()*H,vx:0,vy:0,acc:0});
  const strength=150*activity; // 和之前一样
  const brownian=50*(1-activity);
  const damp=0.92;
  const waveSpeed=130;
  const dt=1; // 像 Processing 一样，dt=1

  let waveTime=0;
  const breathe=0.30+bass*0.30;
  const wobble=0.08+high*0.30;
  const eccentricity=(centroid??0)*0.3;
  ...
  for(let f=0;f<frames;f++){
    waveTime+=dt*(0.1+activity*0.2); // dt=1，waveTime 增长更快
    const sources=getSources(count,breathe,waveTime,wobble,eccentricity);
    ...
    for(const p of particles){
      ...
      if(strength>0.5){
        ...
        p.vx+=field.gx*push*dt; // dt=1
        p.vy+=field.gy*push*dt; // dt=1
      }
      if(brownian>0.5){
...
draw(simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800), 'dt1-bass');
draw(simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800), 'dt1-high');
draw(simulate(5, 0.6, 0.7, 0.8, 0.6, 1.0, 800), 'dt1-mix');
