...
function simulate(count, bass, high, mid, centroid, activity, frames, name, sym=1) {
  ...
  for(let f=0;f<frames;f++){
    ...
    for(const p of particles){
      ...
      function drawRect(x,y){
        ...
        if(acc>0.35){
          const glow=size*2.2, gh=glow*0.5;
          ctx.fillStyle=`rgba(170,180,185,${alpha*0.22})`;
          ctx.fillRect(x-gh,y-gh,glow,glow);
          ctx.fillStyle=`rgba(170,180,185,${alpha})`;
        }
      }
      ...
      drawRect(p.x,p.y);
      if(sym===2){
        drawRect(CX*2-p.x,p.y);
      }else if(sym===4){
        drawRect(CX*2-p.x,p.y);
        drawRect(p.x,CY*2-p.y);
        drawRect(CX*2-p.x,CY*2-p.y);
      }else if(sym>1){
        const dpx=p.x-CX, dpy=p.y-CY;
        for(let sIdx=1;sIdx<sym;sIdx++){
          const ang=(Math.PI*2*sIdx)/sym;
          const cos=Math.cos(ang), sin=Math.sin(ang);
          drawRect(CX+dpx*cos-dpy*sin, CY+dpx*sin+dpy*cos);
        }
      }
    }
  }
  ...
  console.log('saved',name,'sym=',sym);
}

// sym=1 对比
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'latest-bass-s1', 1);
simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800, 'latest-high-s1', 1);
simulate(5, 0.6, 0.7, 0.8, 0.6, 1.0, 800, 'latest-mix-s1', 1);
simulate(4, 0, 0, 0, 0, 0.0, 800, 'latest-silent-s1', 1);

// sym=4 对比
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'latest-bass-s4', 4);
simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800, 'latest-high-s4', 4);
