...
function getSources(count, breathe) {
  const CX=W/2, CY=H/2, MAX_R=Math.min(CX,CY);
  const sources=[];
...
function simulate(count,bass,high,mid,centroid,activity,frames,name){
  ...
  for(let f=0;f<frames;f++){
    ...
    // FIXED: higher bg alpha to prevent overexposure
    ctx.fillStyle='rgba(10,14,16,0.25)';
    ctx.fillRect(0,0,W,H);
    ...
    for(const p of particles){
      ...
      // FIXED: smaller and dimmer glow
      const glowSize=size*(1.2+acc*1.8);
      const glowAlpha=alpha*(0.05+acc*0.10);
...
simulate(4,0.9,0.2,0.4,0.3,1.0,800,'fix-bass');
simulate(4,0.2,0.8,0.6,0.5,1.0,800,'fix-high');
simulate(4,0,0,0,0,0.0,800,'fix-silent');
