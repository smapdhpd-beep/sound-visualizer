...
function simulate(count, bass, high, mid, centroid, activity, frames, name, opts={}) {
  ...
  const viscosity=opts.viscosity||0.92;
  ...
  for(let f=0;f<frames;f++){
    ...
    for(const p of particles){
      ...
      // 光晕：所有粒子都有，大小和透明度与 acc 成正比
      const glowSize = size * (2.0 + acc * 3.0); // 2x ~ 5x
      const glowAlpha = alpha * (0.12 + acc * 0.25); // 0.03 ~ 0.25
      const gh = glowSize * 0.5;
      ctx.fillStyle=`rgba(170,180,185,${glowAlpha})`;
      ctx.fillRect(p.x-gh,p.y-gh,glowSize,glowSize);
    }
    ctx.globalCompositeOperation='source-over';
  }

  fs.writeFileSync(`test-screenshots/${name}.png`,canvas.toBuffer('image/png'));
  console.log('saved',name,'pcount=',pcount);
}

simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'final2-bass-4k', {pcount:4000});
simulate(4, 0.9, 0.2, 0.4, 0.3, 1.0, 800, 'final2-bass-5k', {pcount:5000});
simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800, 'final2-high-4k', {pcount:4000});
simulate(4, 0.2, 0.8, 0.6, 0.5, 1.0, 800, 'final2-high-5k', {pcount:5000});
simulate(4, 0, 0, 0, 0, 0.0, 800, 'final2-silent-4k', {pcount:4000});
